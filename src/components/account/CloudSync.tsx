  ...
  const body = signedIn ? (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs text-content-faint">
        Workouts, templates, records, and settings sync across your devices. Progress photos stay on
        this device.
      </p>
      ...
      <Sheet open={deleteOpen} onClose={() => setDeleteOpen(false)} title="Delete this account?">
        <p className="text-sm text-content-muted">
          This permanently deletes the cloud copy of your training data <em>and</em> the account
          itself ({email}). This device&rsquo;s local-only data is not touched. This cannot be
          undone — export a backup first if you want to keep your log.
        </p>
        {needsPassword && (
          <input
            type="password"
      ...
      <Sheet
        ...
        title="Import this device's data?"
      >
        <p className="text-sm text-content-muted">
          This copies the device&rsquo;s local training data ({anonData?.sessions ?? 0} sessions,{' '}
          {anonData?.templates ?? 0} workouts) into{' '}
          <span className="font-semibold text-content">{email}</span>,{' '}
          <span className="font-semibold text-content">replacing</span> what the account currently
          holds, and syncs it to the cloud. The device copy is kept.
        </p>
        <div className="mt-5 flex flex-col gap-2">
          <Button
            fullWidth
          ...
          <Button variant="ghost" fullWidth onClick={() => setConfirmImport(false)}>
            Cancel
          </Button>
        </div>
      </Sheet>
    ...
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs text-content-muted">
        ...
        <span className="font-semibold text-content">{pendingEmail}</span>. Open it, then sign in.
        Check your spam folder — the confirmation email can take a few minutes.
      </p>
    ...
    <form className="flex flex-col gap-3" onSubmit={submit}>
      <p className="text-xs text-content-faint">
        Sign in to back up your training and sync it across devices. The app works fully offline
        without an account.
      </p>
      <button
        type="button"
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden focusable="false">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.7-6.1 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.9 1.2 8 3.1l5.7-5.7C34.5 6.5 29.6 4.5 24 4.5 13.2 4.5 4.5 13.2 4.5 24S13.2 43.5 24 43.5c11 0 19.5-8 19.5-19.5 0-1.3-.1-2.3-.4-3.5z"
      />
      <path
        fill="#FF3D00"
      ...
      <path
        fill="#4CAF50"
        d="M24 43.5c5.5 0 10.3-1.9 13.8-5.1l-6.4-5.4C29.4 34.6 26.9 35.5 24 35.5c-5.2 0-9.6-3.3-11.3-8l-6.5 5C9.6 39 16.2 43.5 24 43.5z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.3 4.3-4.3 5.6l6.4 5.4C39.9 42.1 43.5 34.5 43.5 24c0-1.3-.1-2.3-.4-3.5z"
      />
    </svg>
  );
}
